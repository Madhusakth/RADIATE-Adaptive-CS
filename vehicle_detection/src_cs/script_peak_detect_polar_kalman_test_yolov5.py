...
if args.sr==10:
    input_folder='10-net_output-polar-kalman-yolov5'
    output_folder='10-net_output_idx-polar-kalman-yolov5'
elif args.sr==20:
    input_folder='20-net_output-polar-kalman-yolov5'
    output_folder='20-net_output_idx-polar-kalman-yolov5'
else:
    input_folder='30-net_output-polar-kalman-yolov5'
    output_folder='30-net_output_idx-polar-kalman-yolov5'
...
min_tracks = 4 #4 #2  test-4 3
max_age = -4 #-4 #-2   test-4 -3
...
full_samp = [1,11,21,31,41]#[1,21,41,61]
def kalman_track():
    final_box = []
    ...
    if args.npy_name in full_samp: #== 1:
        ...
        for idx, box in enumerate(net_output):
                ...
                xx = tmp_trk.x_state
                xx = xx.T[0].tolist()
                #xx = [xx[0], xx[2], xx[4], xx[6]]   #bbox change#
        ...
        for pair in final_pair:
                ...
                if kalman[pair[1]][2] > min_tracks:
                    final_box.append(([xx[0],xx[1],xx[2],xx[3]],1))   #bbox change#
        ...
        for i in unmatched_detections:
                ...
                xx =[xx[0], xx[2], xx[0]+xx[4], xx[2]+xx[6]] ################################################[xx[0], xx[2], xx[4], xx[6]]   ############CHECK
