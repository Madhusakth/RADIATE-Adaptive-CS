...
sr=10
#scene="night_1_4"
folder='10-final-rad-info-polar-cfar-2'
for scene in $scenes
do
for i in {2..21}
do
   ...
   python3 script_peak_detect_cfar.py --npy_name=$prev --scene=$scene --sr=$sr
   ...
   #matlab -nodesktop -nosplash -c 3221@harrison.ece.utexas.edu -r "var1='$scene';var2='$folder'; run compressed_sensing_radar_pcd_bash_polar_final($i,var1,var2,$sr)"
   fi
done
done
