...
function compressed_sensing_radar_pcd_bash_polar(k,scene,radar_output,sr)
    ...
    if sr==10
    	point_radDir = char(strcat(myDir,'/10-net_output_idx-polar-yolov5/'))
	samp_rate=0.1
    elseif sr==20
    	point_radDir = char(strcat(myDir,'/20-net_output_idx-polar-yolov5/'))
	samp_rate=0.2
    else
	point_radDir = char(strcat(myDir,'/30-net_output_idx-polar-yolov5/'))    
	samp_rate=sr*0.01
    end
