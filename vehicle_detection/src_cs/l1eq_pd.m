...
while (~done)
  ...
  if (largescale)
    ...
    if all(val>0)
        ...
        opts.POSDEF = true;
        disp('H11p matrix is not positive definite');
        H11p = H11p + 1e-8*eye(length(H11p));
	disp('Matrix non pos def.  Returning previous iterate.  (See Section 4 of notes for more information.)');
        xp = x;
        return
    end
    ...
    %else
    %    opts.POSDEF = true;
    %    disp('w1p matrix is not positive definite');
    %    w1p = w1p + 1e-8*eye(length(w1p));
    %end
