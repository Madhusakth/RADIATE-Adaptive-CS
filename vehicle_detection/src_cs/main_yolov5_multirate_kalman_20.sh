...
#scenes='snow_1_0 fog_6_0'
folder='20-yolov5l-kalman-10-multi-20-anchor-40-test-9' #-no-object-80'
sr=20
full_sr=40
...
do
	...
	do
	   ...
	   if exists_in_list "$full_sample" " " $i; then
		   ...
		   python test_yolov5_kalman.py --weights=../../../RADIATE_yolov5/yolov5/runs/train/exp7/weights/best.pt --imgsz=1280 --source=../data/radiate/fog_6_0/Navtech_Cartesian/000001.png --data=../../../RADIATE_yolov5/yolov5/data/radcoco.yaml --radar_id=$prev --scene=$scene --folder=$folder --sr=$sr
		   cd /home/ms75986/Desktop/Qualcomm/RADIATE/radiate_sdk/vehicle_detection/src_cs
		   source ../../../../detectron2-venv/bin/activate
		   python3 script_peak_detect_polar_kalman_test_yolov5.py --npy_name=$prev --scene=$scene --sr=$sr --folder=$folder
		   echo "Processing $i th image $scene"
		   matlab -nodesktop -nosplash -c 3221@harrison.ece.utexas.edu -r "var1='$scene';var2='$folder'; run compressed_sensing_radar_pcd_bash_polar_kalman_yolov5($i,var1,var2,$sr)"
	   fi
           fi
	done
done
